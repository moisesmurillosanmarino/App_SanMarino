...
public class GalponService : AppInterfaces.IGalponService
{
    ...
    public async Task<CommonDtos.PagedResult<GalponDtos.GalponDetailDto>> SearchAsync(GalponDtos.GalponSearchRequest req)
    {
        ...
        if (!string.IsNullOrWhiteSpace(req.Search))
        {
            ...
            q = q.Where(g => g.GalponId.ToLower().Contains(term) || g.GalponNombre.ToLower().Contains(term));
        }
    ...
    {
        var q = _ctx.Galpones.AsNoTracking().Where(g => g.CompanyId == _current.CompanyId && g.GalponId == galponId && g.DeletedAt == null);
    ...
    {
        var q = _ctx.Galpones.AsNoTracking().Where(g => g.CompanyId == _current.CompanyId && g.DeletedAt == null);
    ...
    {
        var q = _ctx.Galpones.AsNoTracking().Where(g => g.CompanyId == _current.CompanyId && g.DeletedAt == null && g.GranjaId == granjaId && g.NucleoId == nucleoId);
        return await ProjectToDetail(q).ToListAsync();
    }

    // ------------------------------------
    // Métodos básicos (retrocompatibilidad)
    // ------------------------------------
    public async Task<IEnumerable<GalponDto>> GetAllAsync() => new List<GalponDto>();

    public async Task<GalponDto?> GetByIdAsync(string galponId) => null;

    public async Task<IEnumerable<GalponDto>> GetByGranjaAndNucleoAsync(int granjaId, string nucleoId) => new List<GalponDto>();
    ...
    public async Task<GalponDto> CreateAsync(CreateGalponDto dto)
    {
        ...
        var exists = await _ctx.Galpones.AnyAsync(x => x.CompanyId == _current.CompanyId && x.GalponId == dto.GalponId);
        ...
        return new GalponDto(ent.GalponId, ent.GalponNombre, ent.NucleoId, ent.GranjaId, ent.Ancho, ent.Largo, ent.TipoGalpon);
    }

    public async Task<GalponDto?> UpdateAsync(UpdateGalponDto dto)
    {
        var ent = await _ctx.Galpones.SingleOrDefaultAsync(x => x.CompanyId == _current.CompanyId && x.GalponId == dto.GalponId);
    ...
    {
        var ent = await _ctx.Galpones.SingleOrDefaultAsync(x => x.CompanyId == _current.CompanyId && x.GalponId == galponId);
    ...
    {
        var exists = await _ctx.Farms.AsNoTracking().AnyAsync(f => f.Id == granjaId && f.CompanyId == _current.CompanyId);
    ...
    {
        var exists = await _ctx.Nucleos.AsNoTracking().AnyAsync(n => n.NucleoId == nucleoId && n.GranjaId == granjaId && n.CompanyId == _current.CompanyId);
    ...
    {
        return q.Include(g => g.Farm).Include(g => g.Nucleo).Include(g => g.Company)
            .Select(g => new GalponDtos.GalponDetailDto(
                ...
                g.UpdatedByUserId,
                g.UpdatedAt,
                new SharedDtos.FarmLiteDto(g.Farm.Id, g.Farm.Name, g.Farm.RegionalId, g.Farm.ZoneId),
                new SharedDtos.NucleoLiteDto(g.Nucleo.NucleoId, g.Nucleo.NucleoNombre, g.Nucleo.GranjaId),
                new SharedDtos.CompanyLiteDto(g.Company.Id, g.Company.Name, g.Company.Identifier)
            ));
    }
