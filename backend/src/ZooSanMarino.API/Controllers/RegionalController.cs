...
public class RegionalController : ControllerBase
{
    ...
    [HttpGet("{cia}/{id}")] public async Task<IActionResult> GetById(int cia, int id) =>
        (await _svc.GetByIdAsync(cia, id)) is RegionalDto dto ? Ok(dto) : NotFound();

    [HttpPost] public async Task<IActionResult> Create(CreateRegionalDto dto)
    {
        var cr = await _svc.CreateAsync(dto);
        return CreatedAtAction(nameof(GetById), new { cia = cr.RegionalCia, id = cr.RegionalId }, cr);
    }

    [HttpPut("{cia}/{id}")] public async Task<IActionResult> Update(int cia, int id, UpdateRegionalDto dto)
    {
    ...
    [HttpDelete("{cia}/{id}")] public async Task<IActionResult> Delete(int cia, int id) =>
