...
export interface HierarchicalFilterCriteria {
  companyId?: number | null;
  farmId?: number | null;
  nucleoId?: string | null;
  galponId?: string | null;
  searchTerm?: string;
  selectedLoteId?: number | null; // Changed from string to number
}

export interface HierarchicalFilterState {
  criteria: HierarchicalFilterCriteria;
  selectedLote?: LoteDto | null;
...
export class HierarchicalFilterComponent implements OnInit {
  ...
  @Input() filterLabel = 'Filtros Jerárquicos';
  ...
  // Filtros
  filters = signal<HierarchicalFilterCriteria>({});
  ...
  // Getters para filtros en cascada
  ...
  get selectedCompanyName(): string {
    const companyId = this.filters().companyId;
    return companyId ? this.companies().find(c => c.id === companyId)?.name || '' : '';
  }

  get selectedFarmName(): string {
    const farmId = this.filters().farmId;
    return farmId ? this.farms().find(f => f.id === farmId)?.name || '' : '';
  }

  get selectedNucleoName(): string {
    const nucleoId = this.filters().nucleoId;
    return nucleoId ? this.nucleos().find(n => n.nucleoId === nucleoId)?.nucleoNombre || '' : '';
  }

  get selectedGalponName(): string {
    const galponId = this.filters().galponId;
    return galponId ? this.galpones().find(g => g.galponId === galponId)?.galponNombre || '' : '';
  }

  get selectedLoteName(): string {
    const lote = this.selectedLote();
    return lote ? `${lote.loteId} - ${lote.loteNombre}` : '';
  }
  ...
  onCompanyChange(companyId: number | null): void {
    ...
    const newFilters: HierarchicalFilterCriteria = {
      ...currentFilters,
      companyId,
      selectedLoteId: null
    };
  ...
  onLoteSelect(loteId: number | null): void {
  ...
  private emitChanges(): void {
    ...
    this.stateChange.emit({
      ...
      selectedLote: this.selectedLote(),
      seguimientosCount: this.showSeguimientosCount ? this.getSeguimientosCount() : undefined
    });
  }
  ...
  private getSeguimientosCount(): number {
    // Este método debería ser implementado por el componente padre
    // que conoce el contexto específico (ej: seguimientos, traslados, etc.)
    return 0;
  }
}
