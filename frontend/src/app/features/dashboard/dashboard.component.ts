...
export class DashboardComponent implements OnInit {
  // ==== estados de carga por bloque (lazy)
  loadingSummary = true;
  ...
  // ==================== helpers de error ====================
  private describeError(err: any, fallback = 'No se pudo cargar la información'): string {
  ...
  private clearError(section: SectionKey) {
    this.setError(section, null as unknown as string);
  }
  ...
  private loadSummary() {
    ...
    forkJoin({
      users: this.userSvc.getAll().pipe(
        catchError(err => { summaryErrParts.push('usuarios'); return of([] as UserListItem[]); })
      ),
      farms: this.farmSvc.getAll().pipe(
        catchError(err => { summaryErrParts.push('granjas'); return of([] as FarmDto[]); })
      ),
      lotes: this.loteRepSvc.getLotes().pipe(
        catchError(err => { summaryErrParts.push('lotes'); return of([] as LoteDto[]); })
      ),
    })
    .pipe(finalize(() => { this.loadingSummary = false; }))
    .subscribe(({ users, farms, lotes }) => {
      // índices para correlacionar
      ...
      // (improbable por los catch individuales, pero por si acaso)
  ...
  onActivitiesVisible() {
    ...
    forkJoin({
      lotes: of(this.lotesIndex.size ? Array.from(this.lotesIndex.values()) : []).pipe(
        switchMap(ls => ls.length ? of(ls) : this.loteRepSvc.getLotes().pipe(
    ...
    .subscribe(({ lotes, levante }) => {
      ...
      // Si ambas listas vienen vacías y hay error marcado, se muestra el banner; si no, sin error.
      // No hacemos nada extra aquí.
    }, err => this.setError('activities', this.describeError(err)));
  }
  ...
  onDailyVisible() {
    ...
    this.levanteSvc.getAll().pipe(
      map(rows => {
        ...
        return Array.from(mapByDate.entries())
          .map(([date, entries]) => ({ date, entries } as DailyLog))
    ...
    .subscribe(list => this.dailyLogs = list, err => this.setError('daily', this.describeError(err)));
  }
  ...
  onProductionVisible() {
        ...
        return forkJoin(
          lotes.map(l =>
            this.loteProdSvc.getByLote(l.loteId).pipe(
              catchError(err => {
                // marcamos error pero seguimos con los demás lotes
    ...
    }, err => this.setError('production', this.describeError(err)));
  }
  ...
  onMortalityVisible() {
    ...
    forkJoin({
      ...
      switchMap(({ lotes, levante }) => {
        const prods$ = lotes.length
          ? forkJoin(lotes.map(l => this.loteProdSvc.getByLote(l.loteId).pipe(
              catchError(err => {
    ...
    .subscribe(({ levante, packs }) => {
      ...
      this.mortalities = Array.from(byDate.entries())
        ...
        .sort((a,b) => a.date < b.date ? 1 : -1)
        .slice(0, 10);
    }, err => this.setError('mortality', this.describeError(err)));
  }
